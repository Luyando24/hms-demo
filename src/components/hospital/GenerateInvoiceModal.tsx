...
export default function GenerateInvoiceModal({ isOpen, onClose, onSuccess }: GenerateInvoiceModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (invoiceError) {
      setStatus({ type: 'error', title: 'Invoice Failed', message: invoiceError.message });
      setLoading(false);
      return;
    }
    ...
    setStatus({ type: 'success', title: 'Invoice Generated', message: `Invoice for $${totalAmount.toLocaleString()} has been created.` });
    setLoading(false);
  };
  ...
  return (
    <>
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl w-full max-w-xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
              <h2 className="text-xl font-black text-slate-900">Generate New Invoice</h2>
              <p className="text-sm text-slate-500">Create itemized patient bill.</p>
            </div>
          ...
          <form id="generate-invoice-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button type="button" onClick={addItem} className="text-brand-600 text-xs font-bold flex items-center gap-1 hover:underline">
              ...
              {items.map((item, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  />
                  {items.length > 1 && (
                    <button type="button" onClick={() => removeItem(idx)} className="text-slate-400 hover:text-rose-600 p-1">
                      <Trash2 size={16} />
                    </button>
                  )}
                </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                ...
                <span className="text-xl font-black text-brand-600">${totalAmount.toLocaleString()}</span>
              </div>
            </div>
          </form>
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex gap-3">
            ...
            <button disabled={loading} type="submit" form="generate-invoice-form" className="flex-[2] bg-brand-600 text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-brand-700 transition-colors shadow-lg shadow-brand-500/20 flex items-center justify-center gap-2 disabled:opacity-50">
              {loading ? <Loader2 className="animate-spin" size={18} /> : <><FileText size={18} /> Generate Invoice</>}
            </button>
          </div>
        </div>
      </div>

...
export async function sendFinancialReportEmailAction(input: unknown) {
  try {
    ...
    const [invoicesRes, payrollRes, purchaseOrdersRes, expensesRes] = await Promise.all([
      adminSupabase
        .from('invoices')
    ...
    const outstandingReceivables = Math.max(0, totalInvoiced - realizedCollections);
      ...
      <div class="wrapper">
        ...
        <div class="content">
          <p style="font-size: 13px; color: #334155; margin-top: 0;">
            This automated financial statement consolidates all verified transactions, realized patient revenues, staff payroll disbursements, pharmacy procurements, and general operating expenditures.
          </p>
          ...
          <div class="kpi-grid">
            ...
            <div class="kpi-card">
              ...
              <div class="kpi-val">${fmt(outstandingReceivables)}</div>
            </div>
          </div>
